    ...
    def run(self):
        try:
            ...
            with pyaudio.PyAudio() as p:
                # 只抓 VB-CABLE 的播放端 Loopback。
                # Windows 中应把微信的输出设备设置为：
                #   CABLE Input (VB-Audio Virtual Cable)
                # PyAudioWPatch 会把这个播放设备映射成一个 [Loopback] 输入设备。
                cable_candidates = []
                all_loopbacks = []

                for dev_info in p.get_loopback_device_info_generator():
                    name = str(dev_info.get("name", ""))
                    all_loopbacks.append(name)

                    name_lower = name.lower()
                    if (
                        ("cable" in name_lower or "vb-audio" in name_lower)
                        and "loopback" in name_lower
                    ):
                        cable_candidates.append(dev_info)

                if not cable_candidates:
                    found = "\n".join(f"  - {n}" for n in all_loopbacks) or "  （未发现任何 Loopback 设备）"
                    raise RuntimeError(
                        "没有找到 VB-CABLE 的 Loopback 设备。\n\n"
                        "请检查：\n"
                        "1. 已安装 VB-CABLE 并重启 Windows；\n"
                        "2. Win + R → mmsys.cpl → “播放”中存在 CABLE Input；\n"
                        "3. Windows 音量混合器中，把微信的输出设备设为 CABLE Input；\n\n"
                        "当前检测到的 Loopback 设备：\n"
                        f"{found}"
                    )

                # 优先选择名称中明确包含 CABLE Input 的设备；
                # 若驱动命名略有差异，则使用第一个 VB-CABLE Loopback。
                dev = next(
                    (
                        d for d in cable_candidates
                        if "cable input" in str(d.get("name", "")).lower()
                    ),
                    cable_candidates[0],
                )
                ...
                self.bus.status.emit(
                    f"仅监听微信/VB-CABLE：{dev['name']} | {src_rate}Hz | {channels}声道"
                )
